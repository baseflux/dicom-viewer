...
- Unsorted DICOM series inherit labels derived from their `StudyDescription`, `SeriesDescription`, `Protocol`, and `BodyPartExamined` metadata, with extra body-part keywords (e.g., `BRAIN`, `SPINE`, `TESTIS`, `CHEST`, `LEG`) pushing them into dedicated folders instead of `_unsorted`.
